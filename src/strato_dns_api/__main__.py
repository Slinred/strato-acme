...
@click.option(
    "--log-level",
    "-l",
    type=click.Choice([level for level, _ in logging.getLevelNamesMapping().items()]),
    help="Logging level",
)
@click.pass_context
...
@cli.command()
@click.option(
    "--record-type", "-t", type=click.Choice(["CNAME", "TXT"]), required=True, help="Type of DNS record to add"
)
...
@click.option(
    "--record-type", "-t", type=click.Choice(["CNAME", "TXT"]), required=True, help="Type of DNS record to delete"
)
