...
class StratoDnsApi:
        ...
        try:
            with open(config_file, "r") as f:
                ...
                api = StratoDnsApi(
                    ...
                    credentials=StratoDnsApiCredentials.from_dict(config["credentials"]),
                    log_level=log_level,
                )
    ...
    def __init__(self, location: str, credentials: StratoDnsApiCredentials, log_level=logging.INFO):
        ...
        if location not in self.API_URLS:
    ...
    def login(self) -> bool:
        ...
        request = self._login_2fa(
            request, self._credentials.username, self._credentials.totp_secret, self._credentials.totp_devicename
        )
    ...
    def get_txt_records(self, full_domain: str, package_id: typing.Optional[int] = None) -> list[dict]:
        ...
        request = self._http_session.get(
            self._api_url,
            params={
        ...
        for record in re.finditer(
            ...
            records.append(
                {"prefix": record.group("prefix"), "type": record.group("type"), "value": record.group("value")}
            )

        self._logger.debug(f"Current cname/txt records for '{root_domain}':")
        list(
            self._logger.debug(f"  {item['type']}: {item['prefix']}.{root_domain} = {item['value']}")
            for item in records
        )
    ...
    def remove_txt_record(self, full_domain: str, record_type: str, value: typing.Optional[str] = None) -> bool:
        ...
        for i in reversed(range(len(records))):
            if records[i]["prefix"] == prefix and records[i]["type"] == record_type:
    ...
    def _load_package_id(self, root_domain: str) -> int:
        ...
        self._logger.error(f"Domain {root_domain} not found in strato packages. Using fallback cID=1")
        return 1
